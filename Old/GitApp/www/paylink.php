...
if ($connection) {
    ...
    $importo = $query['totale']; // 5000 = 50,00 EURO (indicare la cifra in centesimi)
    $timeout = 4; // Durata in ore del link di pagamento che verrà generato
    $url = "https://" . filter_input(INPUT_SERVER, 'HTTP_HOST') . "/ordini.html"; // URL dove viene rimandato il cliente al termine del pagamento (prefisso necessario http:// oppure https://)
    $urlBack = "https://" . filter_input(INPUT_SERVER, 'HTTP_HOST') . "/carrello.html"; // URL dove viene rimandato il cliente in caso di annullamento del pagamento (prefisso necessario http:// oppure https://)
    ...
    if (json_last_error() === JSON_ERROR_NONE) {
        ...
        if ($risposta['mac'] == $MACrisposta) {
            ...
            if ($risposta['esito'] == 'OK') {
                ...
                header('Location: '.$risposta['payMailUrl'].'');
            } else {
